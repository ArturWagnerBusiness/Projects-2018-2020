...
ParticleFactory.prototype.tick = function () {
    if (Date.now() > this.lastOutput + this.interval) {
        var particle = new Particle((canvas.width + 100) * Math.random() - 50, -50);
...
ParticleFactory.prototype.recycle = function () {
    this.particles = this.particles.filter(function (item) { return item.y < canvas.height + 50; });
    this.particles = this.particles.filter(function (item) { return item.r > 0.1; });
};
